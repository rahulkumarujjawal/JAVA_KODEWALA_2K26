class Hp_Employee{
public static void main (String args[]){


        System.out.println("Hp_Employee Deatails");

        System.out.println("---------------------------");
        System.out.println("Employee ID   : 101");
        System.out.println("Employee Name : Rahul Ujjawal");
        System.out.println("Designation   : Software Engineer");
        System.out.println("Department    : IT");
        System.out.println("Salary        : 75000");
        System.out.println("Location      : Bangalore");
        System.out.println("Experience    : 4 Years");
        System.out.println("Email ID      : rahul@gmail.com");
        System.out.println("Phone Number  : 9876543210");




}
















}